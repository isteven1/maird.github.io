...
Now that we've looked at if our factors have an effect on birthrate, we can test it on how accurate it is based on residual values. That means how far off was our model from the actual data point. We do this efficently by graphing our residual value versus the actual value
...
This graph shows us that our model is mostly correct with a high freqency at or around zero. This means we predicted (or almost correctly) the correct birthrates for countries.
...
We further look at the correctness of our model by separating our model by year and we notice that our model still hovers around zero each year but is more spread than previoulsly seen. This could be due to factors we did not account for like education or access to healthcare.
