...
In the graph above we see that as birthrate increases, income per person decreases. This could point to a theory that lower income countries have higher birthrates due to lack of education, no family planning resources and no widely available contraception methods. Income per person is a contributing factor to birthrate. As you can see, we took the log of income_per_person. Before transforming this data, the graph was unpresentable and the data points were packed closely together. Taking the log of income_per_person helped spread out the data points while not altering the shape. Generally, a logarithmic transformation is an appropriate transformation to reduce data skew.
...
Heres an in depth review of EDA (don't need to read all of it): https://cran.r-project.org/web/packages/dlookr/vignettes/EDA.html
Heres a shorter, more concise introduction for EDA: https://blog.datascienceheroes.com/exploratory-data-analysis-in-r-intro/
